...
public class DlgAdicionarPizza extends JDialog {
	...
	private boolean testaMaximoAdicionais() {
	   if (grid.getRowCount() >= 3) {
         JOptionPane.showMessageDialog(botaoAdicionarAdicional, "É permitido adicionar no máximo 3 adicionais");
         return true;
      }
	...
	private boolean testaPizzaVegetariana() {
	   boolean vegetariana = false;
	   
	   if (checkSaborUnico.isSelected()) {
         Pizza pizzaUnica = (Pizza) comboPizzaUnica.getSelectedItem();
         
         if (pizzaUnica.getCategoria() == Categoria.VEGETARIANA) {
            vegetariana = true;
         }
      } else {
         Pizza pizza1 = (Pizza) comboPizzaSabor1.getSelectedItem();
         Pizza pizza2 = (Pizza) comboPizzaSabor2.getSelectedItem();
         
         if (pizza1.getCategoria() == Categoria.VEGETARIANA || pizza2.getCategoria() == Categoria.VEGETARIANA) {
            vegetariana = true;
         }
      }
	   
	   if (vegetariana) {
	      Borda borda = (Borda) comboBorda.getSelectedItem();
	      
	      if (borda != Borda.SEM_BORDA_999 && borda != Borda.REQUEIJAO_146 && borda != Borda.QUEIJO_149) {
	         JOptionPane.showMessageDialog(botaoOk, "Não é permitido adicionar a borda " + borda.getDescricao() + " em uma pizza vegetariana");
	         return true;
	      }
	      
	      for (Adicional adicional : pedidoPizza.getAdicionais()) {
	         if (adicional != Adicional.REQUEIJAO_142 && adicional != Adicional.MILHO_143 && adicional != Adicional.AZEITONA_PRETA_144) {
	            JOptionPane.showMessageDialog(botaoOk, "Não é permitido adicionar o adicional " + adicional.getDescricao() + " em uma pizza vegetariana");
	            return true;
	         }
	      }
	   }
