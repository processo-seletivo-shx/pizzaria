...
public class DlgAdicionarPedido extends JDialog {
   ...
   private boolean testaPizzaBebida(JTable pizzaGrid, JTable bebidaGrid) {
      if (pizzaGrid.getRowCount() <= 0 && bebidaGrid.getRowCount() <= 0) {
         JOptionPane.showMessageDialog(botaoOk, "É necessário adicionar pelo menos uma pizza ou bebida!");
         return true;
      }
      return false;
   }

   private boolean testaClienteVazio(JComboBox<Cliente> comboCliente) {
      Cliente cliente = (Cliente) comboCliente.getSelectedItem();
      
      if (cliente != null && cliente.getNome().isEmpty()) {
         JOptionPane.showMessageDialog(botaoOk, "Nenhum cliente foi informado!");
         return true;
      }
      return false;
   }

	private boolean testaSaldoBebida() {

		for (PedidoBebida pedidoBebida : pedido.getPedidoBebidas()) {
			if (pedidoBebida.getQuantidade() > pedidoBebida.getBebida().getSaldo()) {
				JOptionPane.showMessageDialog(botaoOk, "Saldo insuficiente para a bebida " + pedidoBebida.getBebida());
				return true;
			}
		}
		return false;
	...
	private class ActionOk implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			...
			if (testaClienteVazio(comboCliente)) {
				return;
			}
			
			if (testaPizzaBebida(pizzaGrid, bebidaGrid)) {
				return;
			}
	...
	private class ActionRemoverPizza implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			...
			try {
	            ...
	            JOptionPane.showMessageDialog(botaoRemoverPizza, "Nenhuma pizza foi selecionada");
				   return;
				}
	...
	private class ActionRemoverBebida implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
            ...
            if (linhaSelecionada < 0) {
               JOptionPane.showMessageDialog(botaoRemoverPizza, "Nenhuma bebida foi selecionada");
               return;
            }
	...
	private class ActionDetalhes implements ActionListener {
		...
		public void actionPerformed(ActionEvent arg0) {
            ...
            if (linhaSelecionada < 0) {
               JOptionPane.showMessageDialog(botaoDetalhesPizza, "Nenhuma pizza foi selecionada");
               return;
            }
            ...
            if (pedidoPizza.getPizzaUnica() != null) {
               Pizza pizza = pedidoPizza.getPizzaUnica();
               
               JOptionPane.showMessageDialog(botaoDetalhesPizza, pizza.getDescricao() + " (" + pizza.getIngredientes() + ")");
            } else {
               Pizza pizza1 = pedidoPizza.getPizzasSabores().get(0);
               Pizza pizza2 = pedidoPizza.getPizzasSabores().get(1);
               
               JOptionPane.showMessageDialog(botaoDetalhesPizza, "Sabor 1: " + pizza1.getDescricao() + " (" + pizza1.getIngredientes() + ")");
               JOptionPane.showMessageDialog(botaoDetalhesPizza, "Sabor 2: " + pizza2.getDescricao() + " (" + pizza2.getIngredientes() + ")");
            }
			} catch (Exception ex) {
				ex.printStackTrace();
