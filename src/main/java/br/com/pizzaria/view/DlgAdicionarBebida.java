...
public class DlgAdicionarBebida extends JDialog {
	...
	private class ActionOk implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			...
			if(testaNaoPositivo(textQuantidade, "quantidade")) {
			   return;
			}
	...
	private boolean testaNaoInteiro(JTextField field, String label) {
		...
		JOptionPane.showMessageDialog(botaoOk, label + " precisa ser um número inteiro");
		return false;

	}
	
   private boolean testaNaoPositivo(JTextField field, String label) {
      int inteiro = Integer.parseInt(field.getText());
      if (inteiro <= 0) {
         JOptionPane.showMessageDialog(botaoOk, label + " precisa ser um número maior que zero");
         return true;
      }
      return false;
   }
